...
class DailyPlan(models.Model):
    user = models.ForeignKey(
        CustomUser, on_delete=models.CASCADE, related_name="daily_plans"
    )
    ...
    def total_calories(self):
        return round(sum(meal.calories for meal in self.plan_meals.all()), 2)

    @property
    def total_protein(self):
        return round(sum(meal.protein for meal in self.plan_meals.all()), 2)

    @property
    def total_carbs(self):
        return round(sum(meal.carbs for meal in self.plan_meals.all()), 2)

    @property
    def total_fat(self):
        return round(sum(meal.fat for meal in self.plan_meals.all()), 2)
...
class PlanMeal(models.Model):
    ...
    daily_plan = models.ForeignKey(
        DailyPlan, on_delete=models.CASCADE, related_name="plan_meals"
    )
    ...
    name = models.CharField(
        ...
        blank=True,
        null=True,
        help_text="Requerido para snack y suplemento",
    )
